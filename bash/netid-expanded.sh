...
#Task 1. Here we are going to instance our flags so that they can be used properly.
verbose="no"
userInterface="ens33"
#Here is the flag looper.

while [ $# -gt 0 ]; do
	case "$1" in
	-v | --verbose )
		#Gather the verbose things...
		#Because this variable will not exist unless we invoke this flag, I decided to instance it above.
		#That way the flag will exist and can be acted upon regardless of the option.
		echo "Verbose mode on!"
		verbose="yes"
	;;
	-i | --interface )
	shift #Shift away from -i in order to read the user specified interface.
	userInterface=$1
	echo "User specified interface is: $1"
	;;
	* ) #If no flags have been set, check if $1 is empty. If it is not empty, assume the user is looking for some interface information. If it is, the script will run with default values.
...
#Task 2. Let's fetch the interface information in a loop, store it in a variable, and output it below.
#First, instancing the array so that we can add to it.
arrayInterface=()
#Fetch the list of interfaces, cut the interface from the list, get rid of the colon, and clean up the
# extra lines.
for iface in $(ip addr | cut -d ' ' -f2 | tr [:] ' ' | awk NF)
do
	#If the current iface is the loopback, don't add it.
	if [ "$iface" != "lo" ]
		then
			#Add it to the array of interfaces.
			arrayInterface+=("$iface")
	fi
done
echo "============================="
echo "This computer has the following interfaces: ${arrayInterface[@]}"
echo "============================="
#####
...
#####
#We already have an array that contains a proper list of the interfaces we want to fetch data for.
#We can put the below into one giant "for" loop that attempts to fetch the information for each interface.
for iface in "${arrayInterface[@]}"
do
	# define the interface being summarized
	#interface=$userInterface
	#Using "echo" here will trim any whitespace just in case!
	#Also I'm being lazy and not switching the variable names around.
	#Fetch the data for the current interface in the list and output it, then move onto the next.
	interface=$(echo $iface)
